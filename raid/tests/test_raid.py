...
class RaidTest(APITestCase):
    ...
    def test_start_raid_success_case(self):
    ...
    def test_start_raid_success_case2(self):
    ...
    def test_start_raid_fail_case(self):
    ...
    def test_end_raid_success_case(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, {'message': '레이드가 정상적으로 종료 되었습니다.', 'raid_history_id': raid_history.id})
        self.assertEqual(cache.get('can_enter'), True)

    def test_end_raid_fail_case(self):
